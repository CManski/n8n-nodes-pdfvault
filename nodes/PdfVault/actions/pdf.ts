...
export async function executePdfOperation(
	...
	if (operation === 'merge') {
		const binaryProps = (this.getNodeParameter('binaryProperties', itemIndex) as string)
			.split(',')
			.map((s) => s.trim());

		const files = [];
		for (let i = 0; i < binaryProps.length; i++) {
			const { buffer, binaryData } = await getBinaryDataBuffer(this, itemIndex, binaryProps[i]);
			files.push({ fieldName: `file${i === 0 ? '' : i}`, binaryData, buffer });
		}

		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/merge', files);
		...
		const ranges = this.getNodeParameter('ranges', itemIndex) as string;
		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/split', [
		...
		const { buffer, binaryData } = await getBinaryDataBuffer(this, itemIndex, this.getNodeParameter('binaryProperty', itemIndex) as string);
		const quality = this.getNodeParameter('quality', itemIndex, 'medium') as string;
		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/compress', [
			{ fieldName: 'file', binaryData, buffer },
		], { quality });
		...
		const options = this.getNodeParameter('watermarkOptions', itemIndex, {}) as IDataObject;

		const additionalFields: IDataObject = { text, ...options };
		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/watermark', [
			{ fieldName: 'file', binaryData, buffer },
		], additionalFields);
		...
		const { buffer, binaryData } = await getBinaryDataBuffer(this, itemIndex, this.getNodeParameter('binaryProperty', itemIndex) as string);
		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/flatten', [
			{ fieldName: 'file', binaryData, buffer },
		]);
		...
		const { buffer, binaryData } = await getBinaryDataBuffer(this, itemIndex, this.getNodeParameter('binaryProperty', itemIndex) as string);
		const metadataFields = this.getNodeParameter('metadataFields', itemIndex, {}) as IDataObject;

		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/metadata', [
			{ fieldName: 'file', binaryData, buffer },
		], metadataFields);
		...
		const { buffer, binaryData } = await getBinaryDataBuffer(this, itemIndex, this.getNodeParameter('binaryProperty', itemIndex) as string);
		const options = this.getNodeParameter('pageNumberOptions', itemIndex, {}) as IDataObject;

		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/pagenumbers', [
			{ fieldName: 'file', binaryData, buffer },
		], options);
		...
		const binaryProp = this.getNodeParameter('binaryProperty', itemIndex) as string;
		const imageProp = this.getNodeParameter('imageBinaryProperty', itemIndex) as string;
		const placements = this.getNodeParameter('placements', itemIndex) as string;
		const opacity = this.getNodeParameter('imageOpacity', itemIndex) as number;

		const { buffer: pdfBuffer, binaryData: pdfBinary } = await getBinaryDataBuffer(this, itemIndex, binaryProp);
		const { buffer: imgBuffer, binaryData: imgBinary } = await getBinaryDataBuffer(this, itemIndex, imageProp);

		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/image', [
		...
		if (response.downloadUrl) {
			const resultBuffer = await downloadPdfResult.call(this, response.downloadUrl as string);
		...
		const { buffer, binaryData } = await getBinaryDataBuffer(this, itemIndex, this.getNodeParameter('binaryProperty', itemIndex) as string);
		const cropBox = this.getNodeParameter('cropBox', itemIndex) as string;
		const pages = this.getNodeParameter('cropPages', itemIndex) as string;

		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/crop', [
			{ fieldName: 'file', binaryData, buffer },
		], { cropBox, pages });
		...
		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/delete-pages', [
			{ fieldName: 'file', binaryData, buffer },
		], { pages });
		...
		const pages = this.getNodeParameter('rotatePages', itemIndex) as string;

		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/rotate', [
			{ fieldName: 'file', binaryData, buffer },
		...
		const binaryProps = (this.getNodeParameter('imageBinaryProperties', itemIndex) as string)
			.split(',')
			.map((s) => s.trim());
		const pageSize = this.getNodeParameter('pageSize', itemIndex) as string;
		...
		const response = await pdfVaultApiRequestWithFiles.call(this, 'POST', '/v1/image-to-pdf', files, { pageSize });
		...
		const response = await pdfVaultApiRequest.call(this, 'POST', '/v1/url-to-pdf', { url, format });
