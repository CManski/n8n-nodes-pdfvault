...
exports.executeCommentOperation = executeCommentOperation;
...
async function executeCommentOperation(itemIndex) {
    const operation = this.getNodeParameter('operation', itemIndex);
    const returnData = [];
    if (operation === 'list') {
        const fileId = this.getNodeParameter('fileId', itemIndex);
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'GET', '/v1/comments', undefined, { fileId });
        const comments = Array.isArray(response) ? response : (response.comments ?? response);
        ...
        const position = this.getNodeParameter('position', itemIndex, {});
        const body = { fileId, content };
        if (position.pageNumber)
            body.pageNumber = position.pageNumber;
        if (position.xPosition)
            body.xPosition = position.xPosition;
        if (position.yPosition)
            body.yPosition = position.yPosition;
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'POST', '/v1/comments', body);
        returnData.push({ json: response });
    }
    else if (operation === 'reply') {
        const fileId = this.getNodeParameter('replyFileId', itemIndex);
        const parentId = this.getNodeParameter('parentId', itemIndex);
        const content = this.getNodeParameter('replyContent', itemIndex);
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'POST', '/v1/comments', { fileId, content, parentId });
        returnData.push({ json: response });
    }
    else if (operation === 'update') {
        const commentId = this.getNodeParameter('commentId', itemIndex);
        const content = this.getNodeParameter('updateContent', itemIndex);
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'PATCH', `/v1/comments/${commentId}`, { content });
        returnData.push({ json: response });
    }
    else if (operation === 'delete') {
        const commentId = this.getNodeParameter('commentId', itemIndex);
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'DELETE', `/v1/comments/${commentId}`);
        returnData.push({ json: response });
    }
    else if (operation === 'resolve') {
        const commentId = this.getNodeParameter('commentId', itemIndex);
        const resolved = this.getNodeParameter('resolved', itemIndex);
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'PATCH', `/v1/comments/${commentId}`, { resolved });
        returnData.push({ json: response });
    }
    return returnData;
}
