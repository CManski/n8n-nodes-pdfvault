...
exports.esignFields = exports.esignOperations = void 0;
exports.executeEsignOperation = executeEsignOperation;
const GenericFunctions_1 = require("../GenericFunctions");
exports.esignOperations = [
    {
...
async function executeEsignOperation(itemIndex) {
    ...
    if (operation === 'create') {
        ...
        const recipientEmail = this.getNodeParameter('recipientEmail', itemIndex);
        const zones = this.getNodeParameter('zones', itemIndex);
        const options = this.getNodeParameter('esignOptions', itemIndex, {});
        const additionalFields = { recipientEmail, zones };
        if (options.recipientName)
            additionalFields.recipientName = options.recipientName;
        if (options.documentName)
            additionalFields.documentName = options.documentName;
        if (options.signers)
            additionalFields.signers = options.signers;
        if (options.signingMode)
            additionalFields.signingMode = options.signingMode;
        const response = await GenericFunctions_1.pdfVaultBearerRequestWithFiles.call(this, 'POST', '/esign/create', [
            { fieldName: 'file', binaryData, buffer },
        ], additionalFields);
        returnData.push({ json: response });
    }
    else if (operation === 'list') {
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'GET', '/esign/requests');
        ...
        const esignId = this.getNodeParameter('esignId', itemIndex);
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'DELETE', `/esign/${esignId}`);
        returnData.push({ json: response });
    }
    else if (operation === 'downloadSigned') {
        const esignId = this.getNodeParameter('esignId', itemIndex);
        const includeCertificate = this.getNodeParameter('includeCertificate', itemIndex);
        const path = includeCertificate ? `/esign/${esignId}/download?certificate=true` : `/esign/${esignId}/download`;
        const pdfBuffer = await GenericFunctions_1.downloadPdfResult.call(this, path);
        ...
        const esignId = this.getNodeParameter('esignId', itemIndex);
        const pdfBuffer = await GenericFunctions_1.downloadPdfResult.call(this, `/esign/${esignId}/certificate`);
        const binaryItem = await this.helpers.prepareBinaryData(pdfBuffer, 'certificate.pdf', 'application/pdf');
        returnData.push({ json: { success: true, esignId }, binary: { data: binaryItem } });
    }
    else if (operation === 'audit') {
        const esignId = this.getNodeParameter('esignId', itemIndex);
        const response = await GenericFunctions_1.pdfVaultBearerRequest.call(this, 'GET', `/esign/${esignId}/audit`);
        returnData.push({ json: response });
    }
    return returnData;
}
