...
const PdfVault_node_1 = require("../nodes/PdfVault/PdfVault.node");
const PdfVaultApi_credentials_1 = require("../credentials/PdfVaultApi.credentials");
describe('PdfVault Node', () => {
    let node;
    beforeEach(() => {
        node = new PdfVault_node_1.PdfVault();
    });
    ...
    it('should define all 11 resources', () => {
        ...
        const options = resourceProp.options;
    ...
    it('should define all 13 PDF operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('pdf'));
    ...
    it('should define all 7 template operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('template'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
    ...
    it('should define all 7 QR code operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('qr'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
    ...
    it('should define all 6 short URL operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('url'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
        expect(opValues).toContain('create');
    ...
    it('should define all 7 AI operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('ai'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
    ...
    it('should define all 6 eSign operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('esign'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
        expect(opValues).toContain('create');
    ...
    it('should define all 5 webhook operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('webhook'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
        expect(opValues).toContain('create');
    ...
    it('should define all 8 team operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('team'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
        expect(opValues).toContain('create');
    ...
    it('should define all 12 workspace operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('workspace'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
        expect(opValues).toContain('create');
    ...
    it('should define all 5 eSign bulk operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('esignBulk'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
        expect(opValues).toContain('create');
    ...
    it('should define all 6 comment operations', () => {
        const operationProp = node.description.properties.find((p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('comment'));
        expect(operationProp).toBeDefined();
        const options = operationProp.options;
        const opValues = options.map((o) => o.value);
        expect(opValues).toContain('create');
...
describe('PdfVaultApi Credentials', () => {
    let credentials;
    beforeEach(() => {
        credentials = new PdfVaultApi_credentials_1.PdfVaultApi();
    });
...
//# sourceMappingURL=PdfVault.node.test.js.map